...
**关键约束**：站点在 `setup()` 时一次性生成，不再变化；线路在规划阶段一次性建好并锁定，运行期间不变。因此：
- 节点集合（站点）在训练期间固定，图拓扑不变
- 空间邻接边建一次即可复用
- 线路边在规划阶段结束后固定，调度阶段不变
...
1. **空间邻接边**：通过 K-近邻（K=5）连接空间上相近的站点。边特征：归一化欧氏距离（1维）。
   - 在 `setup()` 后只需构建一次，所有站点固定不变。
2. **线路边**：同一线路上的相邻站点。边特征：所属线路 id one-hot（7维）+ 线路上列车数/总列车数（1维）。
   - 在规划阶段由线路规划器决定，`lock_lines()` 后不再变化。
...
**图构建优化**：由于节点集合固定，空间邻接边只需在 episode 开始时计算一次。线路边在规划阶段结束后固定。调度阶段每次决策时只需更新节点动态特征（候车人数、拥堵风险等），图结构本身不变。
...
| 特征 | 维度 | 说明 |
|------|------|------|
| tick_in_day / day_length | 1 | 日内时间进度 (0~1) |
...
| avg_waiting_time | 1 | 平均等待时间 / day_length |
...
| status_onehot | 6 | 列车状态码 (1=落客 2=上客 3=空闲 4=运行 5=调车 6=等待) |
| direction | 1 | 运行方向 |
...
- **图构建**: 站点固定，空间边在 `setup()` 时构建一次；线路边在 `lock_lines()` 后固定，调度阶段图结构不变
